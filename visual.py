...
def extract_features_from_model(model, data_dir, img_size=(224, 224), max_images=200):
    """Extract features from the trained ResNet50 model (before the final classification layer)"""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.eval()
    
    # Create a model that outputs features instead of predictions
...
tsne_features = TSNE(n_components=2, random_state=42, perplexity=30, n_iter=1000)
